...
?>
   ...
   $id = $_GET["id"];
   $query="SELECT s.ID,s.Ten,s.date,s.Gia,s.HinhAnh,s.tacgia,s.KhuyenMai,s.giakhuyenmai,s.Mota, n.Ten as Tennhasx,s.Manhasx,s.soluongkho,s.Madm,d.Madanhmuc,d.Tendanhmuc
   from sanpham s 
   LEFT JOIN nhaxuatban n on n.ID = s.Manhasx
   LEFT JOIN danhmuc d on d.Madanhmuc = s.Madm
	WHERE  s.id =".$id;
              ...
              <div class="box box-info">
                ...
                <form class="form-horizontal"  method="POST" action="<?php include 'xulusuasp.php'?>" enctype="multipart/form-data">
                  <div class="box-body">
                    ...
                    <div class="col-sm-10">
                    <select class="form-control select2" style="width: 100%;" name="madanhmuc">
                      <option selected="selected" value="<?php echo $row["Madanhmuc"] ?>"><?php echo $row["Tendanhmuc"] ?></option>
                      <?php
                         require '../inc/myconnect.php';
                         $sqls="SELECT * from danhmuc where Madmcha != 0 and Madanhmuc !=".$row["Madanhmuc"];
                         $results = $conn->query($sqls); 
                         if ($results->num_rows > 0) {
                          // output data of each row
                          while($rows = $results->fetch_assoc()) {
                      ?>
                        <option value="<?php echo $rows["Madanhmuc"] ?>"><?php echo $rows["Tendanhmuc"] ?></option>
                      <?php 
                          }
                        }
                      ?>
                    ...
                    <div class="form-group">
                    <label for="inputPassword3" class="col-sm-2 control-label">Ngày xuất bản</label>
                    <div class="col-sm-10">
                    ...
                    <div class="col-sm-10">
                    <select class="form-control select2" style="width: 100%;" name="khuyenmai">
                    <option selected="selected" value="0">Không khuyến mãi</option>
                    ...
                    <div class="form-group">
                    <label for="inputPassword3" class="col-sm-2 control-label">Số lượng kho</label>
                    <div class="col-sm-10">
                    <input type="text" class="form-control"  name="soluongkho" required value="<?php echo $row["soluongkho"] ?>">
                    </div>
                    </div>
                  <div class="form-group">
                  ...
                  <a href="qlysanpham.php"><button type="button" name="cancel" class="btn btn-default">Thoát</button></a>
